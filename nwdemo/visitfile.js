...
var GetComputer_ID = function(){
	var expr = "ldap computer";
	exec(expr, function(error, stdout, stderr){
		if(stderr){
			console.log("error: " + error);
		}
		computer_id = JSON.parse(stdout);
		SetUI();
		SetClick();
		SetDBClick();
		SetMouse();
...
function SetUI(){
	for (var one in computer_id){
	$("ul").append("<li><div id=\"" + computer_id[one] + "\" class=\"image\"><img src=\"computer.png\" width=\"60px\" height=\"60px\"  alt=\"\"><div class=\"explain\">" + computer_id[one] + "</div></div></li>");
	}
};

function SetClick(){
	$("li div").bind('click',function(){
	$("li div").css("background-color","#FFF");
	$("li div").css("color","#000000");
	$(this).css('background-color','#CFE3FC');
	$(this).css('color','#FFF');
	});
};

function SetDBClick(){
	$("li div").bind('dblclick',function(){
		var expr = "nautilus smb://" + $(this).attr("id");
		exec(expr, function(error, stdout, stderr){
		if(stderr){
			console.log("error: " + error);
			}
